...
namespace PT.UI.Areas.Manager.Controllers
{
    ...
    public class ServiceManagerController : Base.Controllers.BaseController
    {
        ...
        public async Task<ResponseModel> CreatePost(ServiceModel use, string categoryIds)
        {
            try
            {
                await CheckValidateSlug(use.Slug, CategoryType.Service, use.Id, use.Language);
                if (ModelState.IsValid)
                {
                    ...
                    await AddSeoLink(CategoryType.Service, data.Language, data.Id, MapModel<SeoModel>.Go(use),data.Name);
        ...
        public async Task<ResponseModel> EditPost(ServiceModel use, int id)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    await UpdateSeoLink(use.ChangeSlug, CategoryType.Service, dl.Id, dl.Language, MapModel<SeoModel>.Go(use), dl.Name);
