...
namespace PT.UI.Areas.Manager.Controllers
{
    ...
    public class PromotionInformationManagerController : Base.Controllers.BaseController
    {
        ...
        public async Task<ResponseModel> CreatePost(PromotionInformationModel use)
        {
            try
            {
                await CheckValidateSlug(use.Slug, CategoryType.PromotionInformation, use.Id, use.Language);
                if (ModelState.IsValid)
                {
                    ...
                    await _iContentPageRepository.CommitAsync();
                     ...
                     await AddSeoLink(CategoryType.PromotionInformation, data.Language, data.Id, MapModel<SeoModel>.Go(use), data.Name);
        ...
        public async Task<ResponseModel> EditPost(PromotionInformationModel use, int id)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    await UpdateSeoLink(use.ChangeSlug, CategoryType.PromotionInformation, dl.Id, dl.Language, MapModel<SeoModel>.Go(use), dl.Name);
