...
namespace PT.UI.Areas.Setting.Controllers
{
    ...
    public class SeoSettingsController : BaseController
    {
        ...
        [HttpGet]
        [AuthorizePermission("Index")]
        public IActionResult Sitemap(string language = "vi")
        {
            return View();
        }
        ...
        {
            Func<IQueryable<Link>, IOrderedQueryable<Link>> functionOrder = null;
            switch (orderby)
            {
                case "name":
                    functionOrder = ordertype == "asc" ? EntityExtention<Link>.OrderBy(m => m.OrderBy(x => x.ObjectId)) : EntityExtention<Link>.OrderBy(m => m.OrderByDescending(x => x.ObjectId));
                    break;
                default:
                    functionOrder = ordertype == "asc" ? EntityExtention<Link>.OrderBy(m => m.OrderBy(x => x.Id)) : EntityExtention<Link>.OrderBy(m => m.OrderByDescending(x => x.Id));
                    break;
            }
        ...
        public async Task<ResponseModel> SitemapEditPost(SeoModel use)
        {
            try
            {
                ...
                await CheckValidateSlug2(use.Slug, dl.Id, dl.Language);
                if (ModelState.IsValid)
                {
                   
                    await UpdateSeoLink(use.ChangeSlug, dl.Type, dl.ObjectId, dl.Language, MapModel<SeoModel>.Go(use), dl.Name);
