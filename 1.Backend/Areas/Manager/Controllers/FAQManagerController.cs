...
namespace PT.UI.Areas.Manager.Controllers
{
    ...
    public class FAQManagerController : Base.Controllers.BaseController
    {
        ...
        public async Task<ResponseModel> CreatePost(FAQModel use)
        {
            try
            {
                await CheckValidateSlug(use.Slug, CategoryType.FAQ, use.Id, use.Language);
                if (ModelState.IsValid)
                {
                    ...
                    await AddSeoLink(CategoryType.FAQ, data.Language, data.Id, MapModel<SeoModel>.Go(use), data.Name);
        ...
        public async Task<ResponseModel> EditPost(FAQModel use, int id)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    await UpdateSeoLink(use.ChangeSlug, CategoryType.FAQ, dl.Id, dl.Language, MapModel<SeoModel>.Go(use), dl.Name);
