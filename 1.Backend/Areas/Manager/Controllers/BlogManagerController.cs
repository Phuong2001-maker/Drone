...
namespace PT.UI.Areas.Manager.Controllers
{
    ...
    public class BlogManagerController : Base.Controllers.BaseController
    {
        ...
        [AuthorizePermission("Index")]
        public async Task<ResponseModel> CreatePost(BlogModel use, string categoryIds)
        {
            try
            {
                await CheckValidateSlug(use.Slug, CategoryType.Blog, use.Id, use.Language);
                if (ModelState.IsValid)
                {
                    ...
                    await AddSeoLink(CategoryType.Blog, data.Language, data.Id, MapModel<SeoModel>.Go(use), data.Name);
        ...
        public async Task<ResponseModel> EditPost(BlogModel use, int id)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    dl.TemplateType = use.TemplateType;
                    _iContentPageRepository.Update(dl);
                    await _iContentPageRepository.CommitAsync();

                    await UpdateSeoLink(use.ChangeSlug, CategoryType.Blog, dl.Id, dl.Language, MapModel<SeoModel>.Go(use), dl.Name);
