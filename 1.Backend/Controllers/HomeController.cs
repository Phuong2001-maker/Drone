...
namespace PT.UI.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(IOptions<List<SeoSettings>> seoSettings, ILinkRepository iLinkRepository, IOptions<BaseSettings> baseSettings, IHostingEnvironment iHostingEnvironment, IContentPageRepository iContentPageRepository, ILinkReferenceRepository iLinkReferenceRepository)
        {
        ...
        public IActionResult Index(string language, string linkData)
        {
            ViewData["linkData"] = Newtonsoft.Json.JsonConvert.DeserializeObject<Link>(linkData);
            return View();
        }
        public IActionResult Page404(string language, string linkData)
        {
            ViewData["linkData"] = Newtonsoft.Json.JsonConvert.DeserializeObject<Link>(linkData);
            return View("_Home404");
        }
        public IActionResult About(string language, string linkData)
        {
            ViewData["linkData"] = Newtonsoft.Json.JsonConvert.DeserializeObject<Link>(linkData);
            return View();
        }
        ...
        public IActionResult Clinic1(string language, string linkData)
        {
            ViewData["linkData"] = Newtonsoft.Json.JsonConvert.DeserializeObject<Link>(linkData);
            return View();
        }

        public IActionResult Clinic2(string language, string linkData)
        {
            ViewData["linkData"] = Newtonsoft.Json.JsonConvert.DeserializeObject<Link>(linkData);
            return View();
        }
        ...
        //public Task<FileStreamResult> Sitemap(string language = "vi")
        //{
        ...
        //[Route("{language}/sitemap_old.xml")]
        //public Task<FileStreamResult> SitemapOld(string language = "vi")
        //{
        //    return GetFileSitemap(language, "rosedentalclinic.com");
        //}

        //[Route("sitemap_old.xml")]
        //public Task<FileStreamResult> SitemapAllOld(string language = "vi")
        //{
        //    return GetFileSitemap("", "rosedentalclinic.com");
        //}
        ...
        private FileStreamResult GetFileRobots(string language)
        {
            ...
            Response.Headers.Add("Content-Disposition", "inline;filename=robots.txt");
        ...
        private async Task<FileStreamResult> GetFileSitemap(string language, string domain = null)
        {
            try
            {
                ...
                string Domain = $"{ AppHttpContext.Current.Request.Scheme }://{Request.Host.ToString()}";
                ...
                foreach (var item in listItem)
                {
                    ...
                    stringBuilder.AppendLine("<lastmod>" + (item.Lastmod == null ? DateTime.Now.ToString("yyyy-MM-ddThh:mm:ss+00:00") : item.Lastmod.ToString("yyyy-MM-ddThh:mm:ss+00:00")) + "</lastmod>");
                ...
                Response.Headers.Add("Content-Disposition", "inline;filename=sitemap.xml");
        ...
        public IActionResult BannerHomePage(string language, bool isvertical)
        {
            return View("BannerHomePage", new BanerHomeModel { Language =  language, Isvertical = isvertical});
        }

        private async Task ChangeDomain()
        {
            var list = await _iContentPageRepository.SearchAsync(false, 0, 0, x => x.Content.Contains("rosedentalclinic.com"));
            foreach(var item in list)
            {
                item.Content = item.Content.Replace("rosedentalclinic.com", "eastrosedental.com");
