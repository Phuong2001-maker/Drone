...
namespace PT.UI.Areas.Manager.Controllers
{
    ...
    public class PageManagerController : Base.Controllers.BaseController
    {
        ...
        [AuthorizePermission("Index")]
        public async Task<ResponseModel> CreatePost(PageModel use)
        {
            try
            {
                await CheckValidateSlug(use.Slug, CategoryType.Page, use.Id, use.Language);
                if (ModelState.IsValid)
                {
                    ...
                    await _iContentPageRepository.CommitAsync();
                     ...
                     await AddSeoLink(CategoryType.Page, data.Language, data.Id, MapModel<SeoModel>.Go(use),data.Name);
        ...
        public async Task<ResponseModel> EditPost(PageModel use, int id)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    await UpdateSeoLink(use.ChangeSlug, CategoryType.Page, dl.Id, dl.Language, MapModel<SeoModel>.Go(use),dl.Name);
